...
from performance_dashboard.utils.helpers import add_time_bucket, safe_divide
...
def _render_conversion_trend(bd):
    """전환값 추이 차트"""
    st.subheader("**전환값 추이**")
    tmp = bd.groupby("bucket")[["cost", "signup_7d", "create_account_7d"]].sum().reset_index()
...
def _render_funnel_conversion_trend(bd):
    ...
    tmp2 = bd.groupby("bucket")[["installs", "signup_7d", "create_account_7d"]].sum().reset_index()
...
def _render_cost_trend(bd):
    ...
    # 집계 & 계산
    cols = ["bucket", "cost", "installs", "signup_7d", "create_account_7d"]
    if "impressions" in bd.columns:
        cols.append("impressions")
    agg = bd[cols].groupby("bucket").sum().reset_index()
